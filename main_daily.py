...
2. Ask Claude for: English gist, Urdu gist, full summary, vocabulary, 50 MCQs.
3. Ask Claude (web search) for CSS-relevant PDF/resource suggestions.
...
def run():
    ...
    print("Generating daily package with Claude...")
    package = ai_processor.make_daily_package(article, headline_pool)

    print("Searching for CSS-relevant PDF resources...")
    ...
    if css_resources:
        telegram_bot.send_message(f"🔎 CSS-relevant PDF/resource suggestions:\n{css_resources}")
